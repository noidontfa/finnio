...
MEDIA_BIN="${MEDIA_BIN:-/usr/local/bin/media}"
...
curl --fail --silent --show-error \
  ...
  --data-urlencode "source_type=${MTX_SOURCE_TYPE:-}" \
  --data-urlencode "source_id=${MTX_SOURCE_ID:-}" \
  "$API_BASE_URL/mediamtx/hooks/ready"

INPUT_URL="rtmp://127.0.0.1:${RTMP_PORT}/${MTX_PATH}"
# OUT="../deployments/tmp/segment-rtmp/${MTX_PATH}"

OUT="/hls/abr/${MTX_PATH}"
exec "$MEDIA_BIN" segment-rtmp -i "$INPUT_URL" -o "$OUT" -t 6 -s "$MTX_PATH"